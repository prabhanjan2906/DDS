...
public class JoinQuery implements Serializable{
    ...
    public JavaPairRDD<Envelope, HashSet<Point>> SpatialJoinQueryCartesian(PointRDD pointRDD, RectangleRDD rectangleRDD) {
        List<Tuple2<Envelope, HashSet<Point>>> tuple2List = new ArrayList<>();
        JavaRDD<Envelope> javaRDD = rectangleRDD.getRawRectangleRDD();
        Iterator<Envelope> iterator =  javaRDD.toLocalIterator();
        while (iterator.hasNext()) {
            Envelope env = iterator.next();
            //0 means query predicate is "contains", 1 means query predicate is "intersects"
            PointRDD pointRDDQuery = RangeQuery.SpatialRangeQuery(pointRDD, env, 0);
            JavaRDD<Point> queryPointsRDD = pointRDDQuery.getRawPointRDD();
            List<Point> collectedPoints = queryPointsRDD.collect();
            tuple2List.add(new Tuple2<>(env, new HashSet<>(collectedPoints)));
        }
        JavaPairRDD<Envelope, HashSet<Point>> javaPairRDD = sc.parallelizePairs(tuple2List);
        return javaPairRDD;
    }
